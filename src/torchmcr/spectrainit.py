...
def simplisma(data, n_components, noise_factor=0.1, normalize=True):
    ...
    for i in range(1, n_components):
        ...
        for j in range(i):
            ...
            # Shape: (1, n_wavenumbers)
            pure_spectrum = data[pure_spectra_indices[j], :].unsqueeze(0)
            ...
            covariance = (
                (data - mean_data.unsqueeze(1))
                * (pure_spectrum - mean_pure.unsqueeze(1))
            ).sum(dim=1) / (n_wavenumbers - 1)
